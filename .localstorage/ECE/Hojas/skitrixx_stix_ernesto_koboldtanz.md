# Skitrixx "Stix Ernesto" Koboldtanz

> Archivo generado automaticamente desde la campana V2 del Toolset.
> No editar manualmente.

## Resumen
- Clase: Kobold Rogue
- Nivel: 9

## Estado actual

- Revision: 0
- Ultima actualizacion: 2026-07-26T03:35:34.464Z

## Datos completos V2

```json
{
    "schemaVersion":  2,
    "id":  "chr_2de8dab74a76cdd8d2b0d37cc84725ea",
    "revision":  0,
    "name":  "Skitrixx \"Stix Ernesto\" Koboldtanz",
    "color":  "#d9bd73",
    "identity":  {
                     "className":  "Kobold Rogue",
                     "level":  9,
                     "experience":  48000,
                     "alignment":  "TN"
                 },
    "abilities":  {
                      "strength":  8,
                      "dexterity":  20,
                      "constitution":  13,
                      "intelligence":  8,
                      "wisdom":  8,
                      "charisma":  16
                  },
    "combat":  {
                   "armorClass":  17,
                   "speed":  "30ft.",
                   "initiative":  "+5",
                   "hitPoints":  {
                                     "current":  52,
                                     "maximum":  59,
                                     "temporary":  0
                                 },
                   "hitDice":  {
                                   "current":  "0",
                                   "formula":  "0d8",
                                   "remaining":  0,
                                   "maximum":  9,
                                   "dieSize":  8
                               },
                   "deathSaves":  {
                                      "successes":  0,
                                      "failures":  0
                                  },
                   "conditions":  {

                                  },
                   "inspiration":  true,
                   "exhaustion":  0
               },
    "proficiencies":  {
                          "weapons":  [
                                          "Arma Marcial",
                                          "Arma Simple"
                                      ],
                          "armor":  "Liviano",
                          "languages":  [
                                            "Comun",
                                            "Dracónico",
                                            "Canto de Ladron"
                                        ],
                          "tools":  [
                                        "Juego de Cartas para Jugar",
                                        "Herramientas de Ladrón"
                                    ]
                      },
    "checks":  {
                   "skills":  {
                                  "acrobatics":  {
                                                     "proficiency":  1,
                                                     "bonus":  0,
                                                     "rollMode":  "normal"
                                                 },
                                  "animalHandling":  {
                                                         "proficiency":  0,
                                                         "bonus":  0,
                                                         "rollMode":  "normal"
                                                     },
                                  "arcana":  {
                                                 "proficiency":  0,
                                                 "bonus":  0,
                                                 "rollMode":  "normal"
                                             },
                                  "athletics":  {
                                                    "proficiency":  0,
                                                    "bonus":  0,
                                                    "rollMode":  "normal"
                                                },
                                  "deception":  {
                                                    "proficiency":  1,
                                                    "bonus":  0,
                                                    "rollMode":  "normal"
                                                },
                                  "history":  {
                                                  "proficiency":  0,
                                                  "bonus":  0,
                                                  "rollMode":  "normal"
                                              },
                                  "insight":  {
                                                  "proficiency":  1,
                                                  "bonus":  0,
                                                  "rollMode":  "normal"
                                              },
                                  "intimidation":  {
                                                       "proficiency":  0,
                                                       "bonus":  0,
                                                       "rollMode":  "normal"
                                                   },
                                  "investigation":  {
                                                        "proficiency":  0,
                                                        "bonus":  0,
                                                        "rollMode":  "normal"
                                                    },
                                  "medicine":  {
                                                   "proficiency":  0,
                                                   "bonus":  0,
                                                   "rollMode":  "normal"
                                               },
                                  "nature":  {
                                                 "proficiency":  0,
                                                 "bonus":  0,
                                                 "rollMode":  "normal"
                                             },
                                  "perception":  {
                                                     "proficiency":  2,
                                                     "bonus":  0,
                                                     "rollMode":  "normal"
                                                 },
                                  "performance":  {
                                                      "proficiency":  0,
                                                      "bonus":  0,
                                                      "rollMode":  "normal"
                                                  },
                                  "persuasion":  {
                                                     "proficiency":  1,
                                                     "bonus":  0,
                                                     "rollMode":  "normal"
                                                 },
                                  "religion":  {
                                                   "proficiency":  0,
                                                   "bonus":  0,
                                                   "rollMode":  "normal"
                                               },
                                  "sleightOfHand":  {
                                                        "proficiency":  2,
                                                        "bonus":  0,
                                                        "rollMode":  "normal"
                                                    },
                                  "stealth":  {
                                                  "proficiency":  2,
                                                  "bonus":  0,
                                                  "rollMode":  "normal"
                                              },
                                  "survival":  {
                                                   "proficiency":  0,
                                                   "bonus":  0,
                                                   "rollMode":  "normal"
                                               }
                              },
                   "savingThrows":  {
                                        "strength":  {
                                                         "proficiency":  0,
                                                         "bonus":  0,
                                                         "rollMode":  "normal"
                                                     },
                                        "dexterity":  {
                                                          "proficiency":  1,
                                                          "bonus":  0,
                                                          "rollMode":  "normal"
                                                      },
                                        "constitution":  {
                                                             "proficiency":  0,
                                                             "bonus":  0,
                                                             "rollMode":  "normal"
                                                         },
                                        "intelligence":  {
                                                             "proficiency":  1,
                                                             "bonus":  0,
                                                             "rollMode":  "normal"
                                                         },
                                        "wisdom":  {
                                                       "proficiency":  0,
                                                       "bonus":  0,
                                                       "rollMode":  "normal"
                                                   },
                                        "charisma":  {
                                                         "proficiency":  0,
                                                         "bonus":  0,
                                                         "rollMode":  "normal"
                                                     }
                                    },
                   "initiative":  {
                                      "bonus":  0,
                                      "rollMode":  "normal"
                                  },
                   "passiveBonuses":  {
                                          "perception":  0,
                                          "investigation":  0,
                                          "insight":  0
                                      }
               },
    "actions":  [
                    {
                        "id":  "act_cd9332b2e25aa45c5702f40423226955",
                        "order":  0,
                        "name":  "Sneak Attack (Ranged)",
                        "categories":  "other",
                        "activation":  "",
                        "reach":  "5ft",
                        "ability":  "dexterity",
                        "proficient":  true,
                        "attackBonus":  0,
                        "damageExpression":  "5d6",
                        "damageBonus":  5,
                        "damageType":  "Piercing",
                        "weaponType":  "A distancia",
                        "properties":  "Small, Single-Handed",
                        "description":  "A brightly Colored Maul",
                        "inventoryItemId":  null,
                        "rollMode":  "normal"
                    },
                    {
                        "id":  "act_8052b38588d9723221f4af360a6e55c8",
                        "order":  1,
                        "name":  "Shortbow",
                        "categories":  [
                                           "attack",
                                           "action"
                                       ],
                        "activation":  "",
                        "reach":  "80ft/320ft",
                        "ability":  "dexterity",
                        "proficient":  true,
                        "attackBonus":  0,
                        "damageExpression":  "1d6",
                        "damageBonus":  0,
                        "damageType":  "Piercing",
                        "weaponType":  "A distancia",
                        "properties":  "Ammunition, Two-Handed",
                        "description":  "A compact and lightweight ranged weapon, ideal for quick shots at medium distances. It is favored for its ease of use and portability, often carried by hunters or scouts.",
                        "inventoryItemId":  null,
                        "rollMode":  "normal"
                    },
                    {
                        "id":  "act_87e38e1c09d076c53be82345809f0857",
                        "order":  2,
                        "name":  "Sneak Attack (Melee)",
                        "categories":  "other",
                        "activation":  "",
                        "reach":  "5ft",
                        "ability":  "dexterity",
                        "proficient":  true,
                        "attackBonus":  2,
                        "damageExpression":  "5d6",
                        "damageBonus":  2,
                        "damageType":  "",
                        "weaponType":  "Cuerpo a cuerpo",
                        "properties":  "Heavy, Two-Handed",
                        "description":  "A brightly Colored Maul",
                        "inventoryItemId":  null,
                        "rollMode":  "normal"
                    },
                    {
                        "id":  "act_b65690cddd556535319bf4ed5f891c3b",
                        "order":  3,
                        "name":  "Espada Corta Viciosa",
                        "categories":  [
                                           "attack",
                                           "action"
                                       ],
                        "activation":  "",
                        "reach":  "5ft",
                        "ability":  "dexterity",
                        "proficient":  true,
                        "attackBonus":  2,
                        "damageExpression":  "1d6",
                        "damageBonus":  2,
                        "damageType":  "Perforante",
                        "weaponType":  "Cuerpo a cuerpo",
                        "properties":  "Sutil, Ligero, Monje",
                        "description":  "Cuando sacas un 20 en tu tirada de ataque con esta arma mágica, el objetivo sufre 7 puntos de daño extra del tipo del arma.",
                        "inventoryItemId":  "inv_5ec27f6fae673969edca3f3a8386a458",
                        "rollMode":  "normal"
                    }
                ],
    "inventory":  [
                      {
                          "id":  "inv_110d2875782898a96e3a3229b5ccac82",
                          "order":  0,
                          "group":  "equipment",
                          "name":  "Pua, hierro",
                          "quantity":  1,
                          "unitWeight":  5,
                          "cost":  {
                                       "quantity":  1,
                                       "unit":  "sp"
                                   },
                          "category":  "equipment",
                          "description":  "",
                          "properties":  {

                                         },
                          "equipped":  false,
                          "attuned":  false,
                          "requiresAttunement":  false,
                          "usable":  false,
                          "consumable":  false,
                          "charges":  null,
                          "armor":  null,
                          "weapon":  null,
                          "bonuses":  {

                                      },
                          "effect":  {
                                         "description":  "",
                                         "active":  false
                                     }
                      },
                      {
                          "id":  "inv_5ec27f6fae673969edca3f3a8386a458",
                          "order":  1,
                          "group":  "equipment",
                          "name":  "Espada Corta Viciosa",
                          "quantity":  1,
                          "unitWeight":  2,
                          "cost":  {
                                       "quantity":  10,
                                       "unit":  "gp"
                                   },
                          "category":  "equipment",
                          "description":  "",
                          "properties":  {

                                         },
                          "equipped":  true,
                          "attuned":  false,
                          "requiresAttunement":  false,
                          "usable":  false,
                          "consumable":  false,
                          "charges":  null,
                          "armor":  null,
                          "weapon":  null,
                          "bonuses":  {

                                      },
                          "effect":  {
                                         "description":  "",
                                         "active":  false
                                     }
                      },
                      {
                          "id":  "inv_f3451ccfa101847572eed1826f0164f1",
                          "order":  2,
                          "group":  "equipment",
                          "name":  "Botas Élficas",
                          "quantity":  1,
                          "unitWeight":  0.1,
                          "cost":  {
                                       "quantity":  1500,
                                       "unit":  "gp"
                                   },
                          "category":  "equipment",
                          "description":  "",
                          "properties":  {

                                         },
                          "equipped":  true,
                          "attuned":  false,
                          "requiresAttunement":  false,
                          "usable":  false,
                          "consumable":  false,
                          "charges":  null,
                          "armor":  null,
                          "weapon":  null,
                          "bonuses":  {

                                      },
                          "effect":  {
                                         "description":  "",
                                         "active":  false
                                     }
                      },
                      {
                          "id":  "inv_022a3f6362216bd79b8c5a58d859ff76",
                          "order":  3,
                          "group":  "equipment",
                          "name":  "Armadura de Cuero Tachonado",
                          "quantity":  1,
                          "unitWeight":  13,
                          "cost":  {
                                       "quantity":  45,
                                       "unit":  "gp"
                                   },
                          "category":  "equipment",
                          "description":  "",
                          "properties":  {

                                         },
                          "equipped":  true,
                          "attuned":  false,
                          "requiresAttunement":  false,
                          "usable":  false,
                          "consumable":  false,
                          "charges":  null,
                          "armor":  null,
                          "weapon":  null,
                          "bonuses":  {

                                      },
                          "effect":  {
                                         "description":  "",
                                         "active":  false
                                     }
                      },
                      {
                          "id":  "inv_cb929039c8d785ffd3fe16c0a770ea6f",
                          "order":  0,
                          "group":  "backpack",
                          "name":  "Túnicas",
                          "quantity":  1,
                          "unitWeight":  4,
                          "cost":  {
                                       "quantity":  1,
                                       "unit":  "gp"
                                   },
                          "category":  "backpack",
                          "description":  "",
                          "properties":  {

                                         },
                          "equipped":  false,
                          "attuned":  false,
                          "requiresAttunement":  false,
                          "usable":  false,
                          "consumable":  false,
                          "charges":  null,
                          "armor":  null,
                          "weapon":  null,
                          "bonuses":  {

                                      },
                          "effect":  {
                                         "description":  "",
                                         "active":  false
                                     }
                      },
                      {
                          "id":  "inv_54dfc3b2e9967a5866a8898a5654c8c5",
                          "order":  1,
                          "group":  "backpack",
                          "name":  "Cristal de Reserva",
                          "quantity":  7,
                          "unitWeight":  1,
                          "cost":  {
                                       "quantity":  0,
                                       "unit":  "cp"
                                   },
                          "category":  "backpack",
                          "description":  "",
                          "properties":  {

                                         },
                          "equipped":  false,
                          "attuned":  false,
                          "requiresAttunement":  false,
                          "usable":  false,
                          "consumable":  false,
                          "charges":  null,
                          "armor":  null,
                          "weapon":  null,
                          "bonuses":  {

                                      },
                          "effect":  {
                                         "description":  "",
                                         "active":  false
                                     }
                      },
                      {
                          "id":  "inv_6612d379d91199e1a09f0054f73afe78",
                          "order":  2,
                          "group":  "backpack",
                          "name":  "Cristal Luciente",
                          "quantity":  5,
                          "unitWeight":  1,
                          "cost":  {
                                       "quantity":  0,
                                       "unit":  "cp"
                                   },
                          "category":  "backpack",
                          "description":  "",
                          "properties":  {

                                         },
                          "equipped":  false,
                          "attuned":  false,
                          "requiresAttunement":  false,
                          "usable":  false,
                          "consumable":  false,
                          "charges":  null,
                          "armor":  null,
                          "weapon":  null,
                          "bonuses":  {

                                      },
                          "effect":  {
                                         "description":  "",
                                         "active":  false
                                     }
                      }
                  ],
    "traits":  [
                   {
                       "id":  "trg_174ef245a769e8524738742d7f6df9db",
                       "order":  0,
                       "title":  "Haste",
                       "collapsed":  false,
                       "traits":  {
                                      "id":  "trt_9c54687356d9819388f640e2d3bbe0e7",
                                      "order":  0,
                                      "name":  "Haste",
                                      "description":  "Haste spell (+1 att/round, +2 AC, +100% speed, +DEX savings)",
                                      "collapsed":  false,
                                      "uses":  {
                                                   "maximum":  0,
                                                   "used":  0,
                                                   "reset":  "none"
                                               },
                                      "adjustment":  {
                                                         "category":  "combatStats",
                                                         "subcategory":  "AC",
                                                         "ability":  "NONE",
                                                         "value":  0,
                                                         "advantage":  false,
                                                         "disadvantage":  false,
                                                         "applyToDerived":  false
                                                     },
                                      "effect":  {
                                                     "description":  "",
                                                     "active":  false
                                                 }
                                  }
                   },
                   {
                       "id":  "trg_6c1117f94213aaaed6e789a5ef7a5a18",
                       "order":  1,
                       "title":  "Equipment",
                       "collapsed":  false,
                       "traits":  {
                                      "id":  "trt_9153c56aee81662aaeeb345b4075f868",
                                      "order":  0,
                                      "name":  "Boots of Elvenkind",
                                      "description":  "bota",
                                      "collapsed":  false,
                                      "uses":  {
                                                   "maximum":  0,
                                                   "used":  0,
                                                   "reset":  "long-rest"
                                               },
                                      "adjustment":  {
                                                         "category":  "skills",
                                                         "subcategory":  "Stealth",
                                                         "ability":  "NONE",
                                                         "value":  3,
                                                         "advantage":  false,
                                                         "disadvantage":  false,
                                                         "applyToDerived":  false
                                                     },
                                      "effect":  {
                                                     "description":  "",
                                                     "active":  false
                                                 }
                                  }
                   },
                   {
                       "id":  "trg_2dedf5509fa7b3c5e185cc316ed15a50",
                       "order":  2,
                       "title":  "temp",
                       "collapsed":  false,
                       "traits":  {
                                      "id":  "trt_c7ee003db7915f771c25ae79bb8a71b6",
                                      "order":  0,
                                      "name":  "temp",
                                      "description":  "opio del vial",
                                      "collapsed":  false,
                                      "uses":  {
                                                   "maximum":  0,
                                                   "used":  0,
                                                   "reset":  "none"
                                               },
                                      "adjustment":  {
                                                         "category":  "attributes",
                                                         "subcategory":  "CHA",
                                                         "ability":  "CHA",
                                                         "value":  3,
                                                         "advantage":  false,
                                                         "disadvantage":  false,
                                                         "applyToDerived":  false
                                                     },
                                      "effect":  {
                                                     "description":  "",
                                                     "active":  false
                                                 }
                                  }
                   }
               ],
    "notes":  [
                  {
                      "id":  "ntg_34d365438145fe74b714124b6bb96920",
                      "order":  0,
                      "title":  "test",
                      "collapsed":  false,
                      "notes":  {
                                    "id":  "not_2550f538bfd3e5884eec48775e646ac6",
                                    "order":  0,
                                    "title":  "aux inventory",
                                    "content":  "piedra\nalgodon\nticket de un bar\nflyer para la gran feria del queso\npalo con estopa\nMonedas de Cu x 50\nLibro de hechizos del goblin\ntrampa arcana del más alto nivel (desintegrar)\nanillo de oro\ncarta con la firma de la orden noble para recuperar el amuleto del oráculo\nReceta de poción de invisibilidad\nAnillo con el sello del gobernador\nSobre con promesa de pago\nKragor el buscador, puede mejorar el arma corta\nEstuche que ya he visto - cadaver charro de la taberna\nBolsa con GP\n7 reserva --\u003e mago\n5 lucientes --\u003e mago\nLibro para magia 101 del mercador arcano\nPergamino de kobolds del mercader arcano\nPapel doblado del jefe de la guardia para ponerle al emo\nAmuleto con dije del kobold tuerto\nPerla tornasolada aeloriana (Mago)\n4 phase spider fangs\nllaves delmausoleo de la ciudad alta\n\n---\n\nSir Edric de Amondear\nEpitafio: “Fue ley, fue luz, fue escudo. Cayó sin rendirse.”\n\nLady Calendra Lys de Norwyn\nEpitafio: “Sus manos sanaban, su juicio pesaba.”\n\nSir Baltras del Alba Eterna\nEpitafio: “En la luz juzgó. En la sombra, no dudó.”\n\nDame Ysmena de Corlanth\nEpitafio: “Donde otros dudaron, ella se adelantó.”\n\nSir Tavric el Implacable\nEpitafio: “Solo dobló la rodilla ante la ley.”\n\nDame Lucille Ferrant\nEpitafio: “Caminó por el filo del juicio.”\n\nSir Galvain Mornclad\nEpitafio: “Habló con su espada, respondió con su fe.”\n\nLady Mariel del Silencio\nCapa de las Juramentadas (permite lanzar silencio 1/día).\n\n---\n\nKobold tuerto: lider que hizo una alianza con humanos\nAntes del paso de montaña de valverde está la otra tibu.\n\n\n",
                                    "tags":  {

                                             }
                                }
                  },
                  {
                      "id":  "ntg_67d8e44ca7c72cc947e18fe27fe0ed07",
                      "order":  1,
                      "title":  "Notas 2",
                      "collapsed":  false,
                      "notes":  {

                                }
                  }
              ],
    "extras":  {

               },
    "taleSpire":  null,
    "currency":  {
                     "copper":  50,
                     "silver":  0,
                     "electrum":  0,
                     "gold":  145,
                     "platinum":  75
                 },
    "spellcasting":  {
                         "ability":  "INT",
                         "selectedLevel":  "2",
                         "levels":  {
                                        "Cantrip":  {
                                                        "spells":  [
                                                                       {
                                                                           "name":  "Mensaje",
                                                                           "prepared":  "false"
                                                                       },
                                                                       {
                                                                           "name":  "Ilusión menor",
                                                                           "prepared":  "false"
                                                                       },
                                                                       {
                                                                           "name":  "Mano de mago",
                                                                           "prepared":  "false"
                                                                       }
                                                                   ],
                                                        "slots":  {

                                                                  }
                                                    },
                                        "1st-level":  {
                                                          "spells":  [
                                                                         {
                                                                             "name":  "Disfrazarse",
                                                                             "prepared":  "0"
                                                                         },
                                                                         {
                                                                             "name":  "Púas plateadas",
                                                                             "prepared":  "0"
                                                                         },
                                                                         {
                                                                             "name":  "Comando",
                                                                             "prepared":  "0"
                                                                         },
                                                                         {
                                                                             "name":  "Hechizar persona",
                                                                             "prepared":  "0"
                                                                         }
                                                                     ],
                                                          "slots":  [
                                                                        false,
                                                                        false,
                                                                        false,
                                                                        false
                                                                    ]
                                                      },
                                        "2nd-level":  {
                                                          "spells":  [
                                                                         {
                                                                             "name":  "Blur",
                                                                             "prepared":  "0"
                                                                         },
                                                                         {
                                                                             "name":  "Sugestión",
                                                                             "prepared":  "0"
                                                                         },
                                                                         {
                                                                             "name":  "Phantasmal Force",
                                                                             "prepared":  "0"
                                                                         }
                                                                     ],
                                                          "slots":  [
                                                                        false,
                                                                        false
                                                                    ]
                                                      },
                                        "3rd-level":  {
                                                          "spells":  {

                                                                     },
                                                          "slots":  {

                                                                    }
                                                      },
                                        "4th-level":  {
                                                          "spells":  {

                                                                     },
                                                          "slots":  {

                                                                    }
                                                      },
                                        "5th-level":  {
                                                          "spells":  {

                                                                     },
                                                          "slots":  {

                                                                    }
                                                      },
                                        "6th-level":  {
                                                          "spells":  {

                                                                     },
                                                          "slots":  {

                                                                    }
                                                      },
                                        "7th-level":  {
                                                          "spells":  {

                                                                     },
                                                          "slots":  {

                                                                    }
                                                      },
                                        "8th-level":  {
                                                          "spells":  {

                                                                     },
                                                          "slots":  {

                                                                    }
                                                      },
                                        "9th-level":  {
                                                          "spells":  {

                                                                     },
                                                          "slots":  {

                                                                    }
                                                      }
                                    },
                         "showUpcast":  true,
                         "attackBonus":  0,
                         "saveDcBonus":  0,
                         "favoriteSpells":  {

                                            },
                         "spells":  [
                                        {
                                            "id":  "spl_1bdd8c330b134fe5c22f66207090a6ea",
                                            "order":  0,
                                            "name":  "Mensaje",
                                            "level":  0,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Mensaje",
                                                               "level":  0,
                                                               "description":  "Señalas con el dedo a una criatura dentro del alcance y susurras un mensaje. El objetivo (y solo el objetivo) escucha el mensaje y puede responder con un susurro que solo tú puedes escuchar. Puedes lanzar este conjuro a través de objetos sólidos si conoces al objetivo y sabes que está al otro lado de la barrera. El silencio mágico, una piedra de 1 pie, 1 pulgada de metal común, una delgada capa de plomo o un bloque de madera de 3 pies bloquean el conjuro. El conjuro no tiene por qué seguir una línea recta y puede viajar libremente en las esquinas o aberturas.",
                                                               "higherLevels":  "",
                                                               "range":  "120 pies",
                                                               "components":  "V, S, M",
                                                               "material":  "Un pequeño fragmento de hilo de cobre.",
                                                               "ritual":  false,
                                                               "duration":  "1 round",
                                                               "concentration":  false,
                                                               "castingTime":  "1A",
                                                               "school":  "Transmutación",
                                                               "classes":  "Bardo, Hechicero, Mago",
                                                               "attackType":  "none",
                                                               "saveAbility":  "",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_024118eaac70a03d0fedfa9af05f4c47",
                                            "order":  1,
                                            "name":  "Ilusión menor",
                                            "level":  0,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Ilusión menor",
                                                               "level":  0,
                                                               "description":  "Creas un sonido o una imagen de un objeto dentro del alcance que dura mientras lo haga el conjuro. La ilusión también termina si la disipas como acción o si vuelves a lanzar el conjuro. Si creas un sonido, su volumen puede ser desde un susurro hasta un grito. Puede ser tu voz, la voz de otra persona, el rugido de un león, un repique de tambores o cualquier otro sonido que elijas. Puedes hacer que el sonido continúe sin cesar mientras dura el conjuro o hacer un sonido discreto en diferentes momentos. Si creas una imagen de un objeto (como una silla, huellas de barro o un cofre pequeño), debe ser menor que un cubo de 5 pies. La imagen no puede reproducir sonidos, luces, olor ni ningún otro efecto sensorial. Interactuar físicamente con la imagen revela que es una ilusión, dado que las cosas pueden atravesarla. Si una criatura usa su acción para examinar el sonido o la imagen, esta puede determinar que es una ilusión si tiene éxito en una prueba de Inteligencia (Investigación) enfrentada a la CD de tu conjuro. Si la criatura discierne la ilusión por lo que es, esta se vuelve borrosa para ella.",
                                                               "higherLevels":  "",
                                                               "range":  "30 pies",
                                                               "components":  "S, M",
                                                               "material":  "Un poco de vellón.",
                                                               "ritual":  false,
                                                               "duration":  "1m",
                                                               "concentration":  false,
                                                               "castingTime":  "1A",
                                                               "school":  "Ilusión",
                                                               "classes":  "Bardo, Hechicero, Brujo, Mago",
                                                               "attackType":  "save",
                                                               "saveAbility":  "int",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_656eeb0ebc212c35f6e78c956f6a1422",
                                            "order":  2,
                                            "name":  "Mano de mago",
                                            "level":  0,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Mano de mago",
                                                               "level":  0,
                                                               "description":  "Una mano espectral aparece flotando en un punto que elijas dentro del alcance. La mano dura mientras lo haga el conjuro o hasta que la disipes como acción. La mano se desvanece si está a más de 30 pies de ti o si vuelves a lanzar este conjuro. Puedes usar tu acción para controlar la mano. Puedes usar la mano para manipular un objeto, abrir una puerta o un recipiente cerrado, guardar o sacar un objeto de un recipiente abierto o verter el contenido de un vial. Puedes mover la mano hasta 30 pies cada vez que la usas. La mano no puede atacar, activar objetos mágicos ni transportar más de 10 libras.",
                                                               "higherLevels":  "",
                                                               "range":  "30 pies",
                                                               "components":  "V, S",
                                                               "material":  "",
                                                               "ritual":  false,
                                                               "duration":  "1m",
                                                               "concentration":  false,
                                                               "castingTime":  "1A",
                                                               "school":  "Conjuración",
                                                               "classes":  "Bardo, Hechicero, Bruijo, Mago",
                                                               "attackType":  "none",
                                                               "saveAbility":  "",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_ad1d2bbcb0c2dcde9ecd68db80840bd9",
                                            "order":  0,
                                            "name":  "Disfrazarse",
                                            "level":  1,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Disfrazarse",
                                                               "level":  1,
                                                               "description":  "Haces que tu apariencia (así como la de tu ropa, armadura, armas y otras pertenencias que lleves) sea diferente hasta que termine el conjuro o hasta que uses tu acción para disiparlo. Puedes parecer 1 pie más alto o más bajo y delgado, gordo o normal. No puedes cambiar tu tipo de cuerpo, así que debes adoptar una forma que tenga la misma disposición básica de miembros. Si no, el alcance de la ilusión depende de ti. Los cambios que provoca este conjuro no aguantan una inspección física. Por ejemplo, si usas este conjuro para añadir un sombrero a tu atuendo, los objetos traspasan el sombrero y cualquiera que lo toque no sentirá nada o sentirá tu cabeza y tu pelo. Si usas este conjuro para parecer más delgado de lo que eres, la mano de alguien que estirara el brazo para tocarte se chocará contra ti aunque aparentemente todavía esté en el aire. Para distinguir si estás disfrazado, una criatura puede usar su acción para inspeccionar tu apariencia y hacer una prueba de Inteligencia (Investigar) enfrentada a la CD de la salvación de tu conjuro.",
                                                               "higherLevels":  "",
                                                               "range":  "Propio",
                                                               "components":  "V, S",
                                                               "material":  "",
                                                               "ritual":  false,
                                                               "duration":  "1Hr",
                                                               "concentration":  false,
                                                               "castingTime":  "1A",
                                                               "school":  "Ilusión",
                                                               "classes":  "Bardo, Clérigo, Hechicero, Mago",
                                                               "attackType":  "save",
                                                               "saveAbility":  "int",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_8d1544e5dd347802f7affdcea1f94261",
                                            "order":  1,
                                            "name":  "Púas plateadas",
                                            "level":  1,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Púas plateadas",
                                                               "level":  1,
                                                               "description":  "Distraes mágicamente a la criatura que ha activado el hechizo y conviertes su incertidumbre momentánea en un estímulo para otra criatura. La criatura que ha activado el hechizo debe volver a tirar el d20 y usar la tirada más baja. Luego puedes elegir una criatura diferente que puedas ver dentro del alcance (puedes elegirla tú mismo). La criatura elegida tiene ventaja en la siguiente tirada de ataque, prueba de característica o tirada de salvación que realice en el plazo de 1 minuto. Una criatura solo puede potenciarse con un uso de este hechizo a la vez.",
                                                               "higherLevels":  "",
                                                               "range":  "60 pies",
                                                               "components":  "V",
                                                               "material":  "",
                                                               "ritual":  false,
                                                               "duration":  "Instantáneos",
                                                               "concentration":  false,
                                                               "castingTime":  "1R",
                                                               "school":  "Encantamiento",
                                                               "classes":  "Hechicero, Mago, Bardo",
                                                               "attackType":  "none",
                                                               "saveAbility":  "",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_0e05166ef2b96b35542ad09aabb5bfb6",
                                            "order":  2,
                                            "name":  "Comando",
                                            "level":  1,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Comando",
                                                               "level":  1,
                                                               "description":  "Pronuncias una orden de una palabra a una criatura que puedas ver dentro del alcance. El objetivo debe superar una tirada de salvación de sabiduría o seguir la orden en su siguiente turno. El conjuro no tiene efecto si el objetivo es un no-muerto, si no entiende tu idioma o si tu orden es directamente dañina para él. A continuación se indican algunas órdenes típicas y sus efectos. Puedes dar una orden distinta a las descritas aquí. Si lo haces, el DM determina cómo se comporta el objetivo. Si el objetivo no puede seguir tu orden, el conjuro termina.\u003cb\u003eAproximarse.\u003c/b\u003e El objetivo se mueve hacia ti por la ruta más corta y directa, y termina su turno si se mueve a menos de 5 pies de ti.\u003cb\u003eDejar caer.\u003c/b\u003e El objetivo suelta lo que esté sosteniendo y luego termina su turno.\u003cb\u003eHuir.\u003c/b\u003e El objetivo pasa su turno alejándose de ti por los medios más rápidos disponibles.\u003cb\u003eArrodillarse.\u003c/b\u003e El objetivo cae boca abajo y luego termina su turno.\u003cb\u003eDetenerse.\u003c/b\u003e El objetivo no se mueve y no realiza ninguna acción. Una criatura voladora se mantiene en el aire siempre que pueda hacerlo. Si debe moverse para mantenerse en el aire, vuela la distancia mínima necesaria para permanecer en el aire.",
                                                               "higherLevels":  "Cuando lanzas este hechizo usando un espacio de hechizo de nivel 2 o superior, puedes afectar a una criatura adicional por cada nivel de espacio superior al 1. Las criaturas deben estar a 30 pies entre sí cuando las seleccionas como objetivo..",
                                                               "range":  "60 pies",
                                                               "components":  "V",
                                                               "material":  "",
                                                               "ritual":  false,
                                                               "duration":  "1 ronda",
                                                               "concentration":  false,
                                                               "castingTime":  "1A",
                                                               "school":  "Encantamiento",
                                                               "classes":  "Clérigo, Paladín, Brujo",
                                                               "attackType":  "save",
                                                               "saveAbility":  "wis",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_b00671bedc809c557952fc6938bb2ed5",
                                            "order":  3,
                                            "name":  "Hechizar persona",
                                            "level":  1,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Hechizar persona",
                                                               "level":  1,
                                                               "description":  "Intentas hechizar a un humanoide que puedes ver dentro del alcance. Este debe hacer una tirada de salvación de Sabiduría, para la que tendrá ventaja si tus compañeros o tú estáis luchando contra él. Si falla, queda hechizado hasta que el conjuro termina o hasta que tus compañeros o tú lo dañáis. La criatura hechizada te ve como un conocido amistoso. Cuando el conjuro termina, la criatura sabe que estaba hechizada por ti.",
                                                               "higherLevels":  "Cuando lanzas este conjuro usando un espacio de conjuro de nivel 2 o superior, puedes elegir como objetivo a una criatura adicional por cada nivel por encima de 1. Las criaturas deben estar a 30 pies o menos entre sí.",
                                                               "range":  "30 Pies",
                                                               "components":  "V, S",
                                                               "material":  "",
                                                               "ritual":  false,
                                                               "duration":  "1 Hora",
                                                               "concentration":  false,
                                                               "castingTime":  "1A",
                                                               "school":  "Encantamiento",
                                                               "classes":  "Bardo, Clérigo, Druida, Hechicero, Brujo, Mago",
                                                               "attackType":  "save",
                                                               "saveAbility":  "wis",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_6af7617e24fc1e2bfe6ba698b3e48c83",
                                            "order":  0,
                                            "name":  "Blur",
                                            "level":  2,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Blur",
                                                               "level":  2,
                                                               "description":  "Your body becomes blurred, shifting and wavering to all who can see you. For the duration, any creature has disadvantage on attack rolls against you. An attacker is immune to this effect if it doesnâ€™t rely on sight, as with blindsight, or can see through illusions, as with truesight.",
                                                               "higherLevels":  "",
                                                               "range":  "Self",
                                                               "components":  "V",
                                                               "material":  "",
                                                               "ritual":  false,
                                                               "duration":  "Up to 1m",
                                                               "concentration":  true,
                                                               "castingTime":  "1A",
                                                               "school":  "Illusion",
                                                               "classes":  "Druid, Sorcerer, Wizard",
                                                               "attackType":  "none",
                                                               "saveAbility":  "",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_aeeb845c804b8569fb661e464140b731",
                                            "order":  1,
                                            "name":  "Sugestión",
                                            "level":  2,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Sugestión",
                                                               "level":  2,
                                                               "description":  "Sugieres mágicamente un curso de actividad (limitado a una frase o dos) a una criatura que puedas ver dentro del alcance y que te pueda oír y entender. Las criaturas que no pueden ser hechizadas son inmunes a este efecto. La sugerencia debe plantearse de tal modo que el curso de la acción suene razonable. Pedirle a una criatura que se apuñale a sí misma, que se arroje sobre una lanza, que se inmole o que haga algo obviamente dañino anula automáticamente el efecto del conjuro. El objetivo debe realizar una tirada de salvación de Sabiduría. Si falla, sigue el curso de acción que hayas descrito lo mejor que puede. La acción sugerida puede continuar mientras dure el conjuro. Si se puede completar en poco tiempo, el conjuro acaba cuando el sujeto termina lo que se le pidió que hiciera. También puedes especificar las condiciones que activarán una tarea especial mientras dure el conjuro. Por ejemplo, puedes sugerir que un soldado le dé su caballo de guerra al primer mendigo que se encuentre. Si la condición no se cumple antes de que el conjuro termine, la actividad no se lleva a cabo. Si tú o cualquiera de tus compañeros daña al objetivo, el conjuro termina.",
                                                               "higherLevels":  "",
                                                               "range":  "30 pies",
                                                               "components":  "V, M",
                                                               "material":  "Una lengua de serpiente y un trozo de panal o una gota de jarabe.",
                                                               "ritual":  false,
                                                               "duration":  "Hasta 8 horas",
                                                               "concentration":  true,
                                                               "castingTime":  "1A",
                                                               "school":  "Encantamiento",
                                                               "classes":  "Bardo, Clérigo, Hechicero, Brujo, Mago",
                                                               "attackType":  "save",
                                                               "saveAbility":  "wis",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2014"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        },
                                        {
                                            "id":  "spl_f3fedd4950943ae8c37579c31a086252",
                                            "order":  2,
                                            "name":  "Phantasmal Force",
                                            "level":  2,
                                            "prepared":  false,
                                            "source":  "bundled",
                                            "definition":  {
                                                               "name":  "Phantasmal Force",
                                                               "level":  2,
                                                               "description":  "You attempt to craft an illusion in the mind of a creature you can see within range. The target makes an Intelligence saving throw. On a failed save, you create a phantasmal object, creature, or other phenomenon that is no larger than a 10-foot Cube and that is perceivable only to the target for the duration. The phantasm includes sound, temperature, and other stimuli.\u003cbr\u003e\u003cbr\u003eThe target can take a Study action to examine the phantasm with an Intelligence (Investigation) check against your spell save DC. If the check succeeds, the target realizes that the phantasm is an illusion, and the spell ends.\u003cbr\u003e\u003cbr\u003eWhile affected by the spell, the target treats the phantasm as if it were real and rationalizes any illogical outcomes from interacting with it. For example, if the target steps through a phantasmal bridge and survives the fall, it believes the bridge exists and something else caused it to fall.\u003cbr\u003e\u003cbr\u003eAn affected target can even take damage from the illusion if the phantasm represents a dangerous creature or hazard. On each of your turns, such a phantasm can deal 2d8 Psychic damage to the target if it is in the phantasmâ€™s area or within 5 feet of the phantasm. The target perceives the damage as a type appropriate to the illusion.",
                                                               "higherLevels":  "",
                                                               "range":  "60ft. / 10ft. Cube",
                                                               "components":  "V, S, M",
                                                               "material":  "(a bit of fleece)",
                                                               "ritual":  false,
                                                               "duration":  "1 Minute",
                                                               "concentration":  true,
                                                               "castingTime":  "1A",
                                                               "school":  "Illusion",
                                                               "classes":  "Bard, Sorcerer, Wizard",
                                                               "attackType":  "save",
                                                               "saveAbility":  "int",
                                                               "damageExpression":  "",
                                                               "upcastDamageExpression":  "",
                                                               "addAbilityModifier":  false,
                                                               "damageType":  "",
                                                               "year":  "2024"
                                                           },
                                            "effect":  {
                                                           "description":  "",
                                                           "active":  false
                                                       }
                                        }
                                    ],
                         "slots":  {
                                       "1":  {
                                                 "maximum":  4,
                                                 "used":  0
                                             },
                                       "2":  {
                                                 "maximum":  2,
                                                 "used":  0
                                             },
                                       "3":  {
                                                 "maximum":  0,
                                                 "used":  0
                                             },
                                       "4":  {
                                                 "maximum":  0,
                                                 "used":  0
                                             },
                                       "5":  {
                                                 "maximum":  0,
                                                 "used":  0
                                             },
                                       "6":  {
                                                 "maximum":  0,
                                                 "used":  0
                                             },
                                       "7":  {
                                                 "maximum":  0,
                                                 "used":  0
                                             },
                                       "8":  {
                                                 "maximum":  0,
                                                 "used":  0
                                             },
                                       "9":  {
                                                 "maximum":  0,
                                                 "used":  0
                                             }
                                   }
                     },
    "metadata":  {
                     "createdAt":  "2026-07-26T03:35:34.464Z",
                     "updatedAt":  "2026-07-26T03:35:34.464Z",
                     "migratedFrom":  "v1"
                 }
}
```
